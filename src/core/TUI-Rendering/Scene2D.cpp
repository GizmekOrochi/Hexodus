...
std::vector<TUI::Pixel> Scene2D::convertScene(int outputHeight, int outputLength) {
    int SceneOrigin_X = (this->Origin()[0] * outputHeight) / 100;
    int SceneOrigin_Y = (this->Origin()[1] * outputLength) / 100;
    
    int SceneEnding_X = (this->Ending()[0] * outputHeight) / 100;
    int SceneEnding_Y = (this->Ending()[1] * outputLength) / 100;
    
    // used for the loop to start from 0,0
    int maxXBuffer = SceneEnding_X - SceneOrigin_X;
    int maxYBuffer = SceneEnding_Y - SceneOrigin_Y;
    
    std::vector<TUI::Pixel> SceneBuffer(maxXBuffer * maxYBuffer);
    
    for (int y = 0; y < maxYBuffer; y++) {
        for (int x = 0; x < maxXBuffer; x++) {
            // Calculate current pixel screen position
            ...
            for (const auto& element : SceneElements_) {
                ...
                int elemOriginX = (elementOrigin[0] * outputHeight) / 1000;
                int elemOriginY = (elementOrigin[1] * outputLength) / 1000;
                //int elemEndingX = (elementEnding[0] * outputHeight) / 1000;
                //int elemEndingY = (elementEnding[1] * outputLength) / 1000;

                int ObjscreenX = elemOriginX + x;
                int ObjscreenY = elemOriginY + y;
                
                // Check if current the x,y pixel is at the x,y location
                if (screenX == ObjscreenX && screenY == ObjscreenY) {
            ...
            size_t index = y * maxXBuffer + x;
            if (foundElement) {
                SceneBuffer[index] = currentBestColor;
            } else {
                SceneBuffer[index] = this->Color();
            }
        }
    }
